select_sites_no_biome <- function(sourceDir, destDir, resp.variable) {
    
    siteDF <- read.csv(paste0(getwd(), "/data/sites_14may.csv"))
    
    myDF <- readRDS(paste0(sourceDir, "/Australia_", resp.variable, "_predictability_decile.rds"))
    ...
    saveRDS(siteDF, paste0(destDir, "/selected_site_", resp.variable, "_no_biome_preditability.rds"))
