...
test<-function(sourceDir, destDir) {
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        v.range <- c(X$r01p_spr, X$r01p_sum, 
                     X$r01p_aut, X$r01p_win)
        ...
        spr_cut = cut(X[, "r01p_spr"], breaks, include.lowest=TRUE,right=TRUE)
        sum_cut = cut(X[, "r01p_sum"], breaks, include.lowest=TRUE,right=TRUE)
        aut_cut = cut(X[, "r01p_aut"], breaks, include.lowest=TRUE,right=TRUE)
        win_cut = cut(X[, "r01p_win"], breaks, include.lowest=TRUE,right=TRUE)
    ...
    write.csv(output,paste0(destDir, "/R01PS_PCM.csv"),row.names=F)
