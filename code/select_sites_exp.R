select_sites_exp <- function(sourceDir, destDir, resp.variable) {
    
    siteDF <- read.csv(paste0(getwd(), "/data/sites_14may.csv"))
    
    myDF <- readRDS(paste0(sourceDir, "/Australia_", resp.variable, "_predictability_exp.rds"))
    ...
    saveRDS(siteDF, paste0(destDir, "/selected_site_", resp.variable, "_preditability_exp.rds"))
